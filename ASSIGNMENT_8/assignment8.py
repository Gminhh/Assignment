...
class Elevator:
    ...
    def floor_up(self):
        if self.current < self.top:
            self.current += 1
            print(f"Thang máy hiện đang ở tầng {self.current}")
...
class Building:
    ...
    def run_elevator(self, elevator_number, target_floor):
        print(f"\nThang máy đang từ tầng {elevator_number} tới tầng {target_floor}")
        self.elevators[elevator_number].go_to_floor(target_floor)

    def fire_alarm(self):
        print("\n Cháy rồi bà con ơi! Tất cả thang máy đang tới tầng 1")
        for i, elevator in enumerate(self.elevators):
            print(f"Thang máy {i} đang tiến tới tầng 1 ")
...
class Race:
    ...
    def print_status(self):
        print(f"\nThông số cuộc đua: {self.name}")
        print(f"{'Xe':10} {'Tốc Độ':6} {'Khoảng cách':8}")
        for car in self.cars:
